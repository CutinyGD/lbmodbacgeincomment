...
void download_list() {
	...
	if (!downloaded) {
        ...
        m_listener.setFilter(req.get("https://raw.githubusercontent.com/CutinyGD/GeometryDashHelpers/refs/heads/main/names.txt"));
		}
	}
		...
		for (const auto& names : YouTubers) {
			...
			if (lower_names == lower_player_name) {
				...
				if (player_comment_small || player_comment_big_area) {
					...
					CCNode* last_letter;
					m_fields->badge = CCSprite::create("lbmodbadge.png"_spr);
					...
					if (m_comment->m_modBadge < 1) {
						...
						if (player_comment_big_area) {
							...
							for (int i = 0; i < text_holder->getChildrenCount(); i++) {
								...
								if (player_comment_big->getColor() == ccColor3B{255,255,255}) player_comment_big->setColor({ 179, 242, 255 });
							}

						}
						else {
							if (player_comment_small->getColor() == ccColor3B{255,255,255}) player_comment_small->setColor({ 179, 242, 255 });
						}
